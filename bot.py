...
# Global tracker: semua order yang pernah dibuat, untuk /cancelall
# Format: { chat_id: [order_dict, order_dict, ...] }
all_user_orders = {}

# Autobuy state (dipakai di auto_check_otp juga)
autobuy_active = {}
...
def req_api(api_key, action, **kwargs):
    ...
    try:
        r = http_session.get(API_BASE, params=params, timeout=10)
        return r.text.strip()
...
def auto_check_otp(chat_id, message_id, orders, api_key, country_key="vietnam", is_autobuy_mode=False, s_idx=1):
    ...
    try:
        while True:
            ...
            for o in orders:
                ...
                try:
                    ...
                    if res.startswith('STATUS_OK'):
                        ...
                        o['status'] = 'cancelled'
                        changed = True
                    elif res.startswith('STATUS_WAIT_RETRY') and ':' in res:
            ...
            now = time.time()
            # Update timer setiap 5 detik supaya ga stuck di layar
            ...
            # Jeda antar pengecekan — stabil 3 detik
            time.sleep(CHECK_INTERVAL) 
...
def cancelall_cmd(message):
    """Cancel semua order yang sedang menunggu OTP sekaligus"""
    chat_id = message.chat.id
    user_id = message.from_user.id
    
    if not is_whitelisted(user_id):
        bot.reply_to(message, f"🔒 *Akses Ditolak*\nID Telegram Anda: `{user_id}`", parse_mode="Markdown")
        return
    ...
    # Cancel semua sekaligus
    processing = bot.reply_to(message, f"⏳ Membatalkan {len(waiting_orders)} order...")
